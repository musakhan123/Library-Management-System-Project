...
//new add
void BST::inOrderForEach(std::function<void(Book*)> func) const
{
    inOrderForEachHelper(root, func);
}
//new add
void BST::inOrderForEachHelper(Node* node,
                               std::function<void(Book*)> func) const
{
    if (!node) return;

    inOrderForEachHelper(node->left, func);
    func(&node->data);
    inOrderForEachHelper(node->right, func);
}
