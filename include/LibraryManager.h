...
public:
    ...
    // GUI-only access (READ-ONLY)
	Stack* getTransactionHistory() const;
	// Undo last transaction (BONUS)
    ...
    void rebuildBorrowedLists();
    //new add // GUI-only accessors (READ-ONLY)
LinkedList* getStudentsList() const;
//new add
BST* getBookList() const;
//new add for csv
bool loadBooksFromCSV(const std::string& filename);
bool loadStudentsFromCSV(const std::string& filename);
//add to csv 
bool saveBooksToCSV(const std::string& filename) const;
bool saveStudentsToCSV(const std::string& filename) const;
//csv transaction 
bool saveTransactionsToCSV(const std::string& filename) const;
bool loadTransactionsFromCSV(const std::string& filename);
//search 
